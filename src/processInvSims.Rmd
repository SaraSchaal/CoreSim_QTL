...
ggplot(data = high.alpha[high.alpha$sim_gen > 10000,], 
    ...
    labs(title = "Effect of Inversions on Local Adaptation - high alpha (0.2)",
           y = "Local Adaptation",
           x = "Generation") +
 # scale_fill_discrete(names = c("Migration", "QTN Mutation Rate")) +
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(10200, NA)) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1))

ggplot(data = low.alpha[low.alpha$sim_gen > 10000,], 
       aes(x = sim_gen, y = localAdaptSA, group = interaction(mig, mu_base))) + 
    geom_line(aes(color = mig, linetype = mu_base), size = 0.75) + 
    facet_wrap(~ mu_inv, labeller = labeller(mu_inv = inv.labs)) + 
    labs(title = "Effect of Inversions on Local Adaptation - low alpha (0.002)",
           y = "Local Adaptation",
           x = "Generation") +
 # scale_fill_discrete(names = c("Migration", "QTN Mutation Rate")) +
  guides(color = guide_legend(title = "Migration Rate"), 
...
ggplot(data = high.alpha[high.alpha$sim_gen > 10000,], 
    ...
    geom_line(aes(color = mig, linetype = mu_base), size = 0.75) + 
    facet_wrap(~ mu_inv, labeller = labeller(mu_inv = inv.labs)) + 
    labs(title = "Effect of Inversions on Fitness - high alpha (0.2)",
           y = "Fitness",
           x = "Generation") +
 # scale_fill_discrete(names = c("Migration", "QTN Mutation Rate")) +
  guides(color = guide_legend(title = "Migration Rate"), 
...
ggplot(data = low.alpha[low.alpha$sim_gen > 10000,], 
    ...
    geom_line(aes(color = mig, linetype = mu_base), size = 0.75) + 
    facet_wrap(~ mu_inv, labeller = labeller(mu_inv = inv.labs)) + 
    labs(title = "Effect of Inversions on Fitness - low alpha (0.002)",
           y = "Fitness",
           x = "Generation") +
 # scale_fill_discrete(names = c("Migration", "QTN Mutation Rate")) +
  guides(color = guide_legend(title = "Migration Rate"), 
