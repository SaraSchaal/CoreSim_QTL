...
not.done.seeds <- NULL#c(3383334, 3383330, 3383318, 3383314)
```


```{r}
folder <- "results/PrelimSims/"
...
count <- 1
for(i in 1:nrow(df.invTimeFreq)){
  count <- count+1
  for(j in 1:nrow(df.invData)){
    if(df.invData$inv_id[j] == df.invTimeFreq$inv_id[i]){
      df.invTimeFreq$inv_length[i] <- df.invData$inv_length[j]
    }
  }
  print(count)
}

dim(merge(df.invData, df.invTimeFreq, by.x = c("seed", "inv_id"), by.y = c("seed", "inv_id"), all.y = TRUE))
...
for(i in 1:nrow(inv.sims)){
    ...
    # step through the population dynamics dataframe
    for(j in 1:nrow(df.invTimeFreq)){
      # step through the different seeds that have the unique parameters
      for(k in 1:reps){
        seedCol <- paste("Seed", k, sep="")
        # if that seed is not an NA then do the next step
        if(!is.na(inv.sims[i, seedCol])){
          ...
          if(df.invTimeFreq$seed[j] == inv.sims[i, seedCol]){
           df.length.average <- rbind(df.length.average, df.invTimeFreq[j,])
  ...
  ## average columns of interest
   # df.averageSubset <- df.average[,vect.aggCols]
    ...
    df.average.inv <- cbind(new.length.average, new.numQTNs.average$num_qtns)
...
ggplot(data = df.invLength.average, 
  ...
  scale_color_manual(values=c("cadetblue1","cadetblue3", "cornflowerblue", "navy")) +
  scale_x_continuous(expand = c(0, 0), limits = c(0, NA)) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, max(df.invTimeFreq$inv_length)))
...
final.invFreq <- subset(df.invTimeFreq[df.invTimeFreq$sim_gen == 20000,])
