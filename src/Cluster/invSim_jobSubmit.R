...
for(i in 1:nrow(params)){
  ...
  fileConn <- file(print(paste("src/submissionFiles/20210930_fixOrigin/", params$seed[i],"_slimInv.sh",sep="")))
  ...
  writeLines(c("#!/bin/bash",
               ...
               paste0("#SBATCH --output=../figures/20210930/clustOut/",params$seed[i],".%j.out"),
               paste0("#SBATCH --error=../figures/20210930/clustOut/",params$seed[i],".%j.err"),
               ...
               paste0("srun slim -d seed=", params$seed[i], " -d muBase=", params$muBase[i], " -d n=",
                      ...
                      params$theta2[i], " -d dom=", params$dom[i], " -m -t 20210920_InversionModel.slim"),
               ...
               paste0("srun slim -d seed=", params$seed[i], " -d muBase=", params$muBase[i], " -d n=",
                      ...
                      params$mig1[i]," -d mig2=", params$mig2[i]," -d burnin=", params$burnin[i], " -d chromNum=",
                      params$chromNum[i]," -d r=", params$r[i], " -d theta1=", params$theta1[i], " -d theta2=", 
                      params$theta2[i], " -d dom=", params$dom[i], " -m -t 20210920_InversionModelnoSel.slim"),
               "wait",
               "echo \"finished no selection sim\"",
               
               paste0("vcftools --vcf ../results/20210930/", params$seed[i], "_InversionVCF.vcf --maf 0.01 --out ../results/20210930/",
                      params$seed[i], "_InversionVCF_MAF01 --recode --recode-INFO-all"),
               
               paste0("vcftools --vcf ../results/20210930/", params$seed[i], "noSel_InversionVCF.vcf --maf 0.01 --out ../results/20210930/",
