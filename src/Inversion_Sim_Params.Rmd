...
## Number of total simulations
  num_sim <- length(mu_base)*length(mu_inv)*length(sigmaK)*length(alpha)*length(reps)*length(N)*length(envar)*length(mig1)*length(recom)*length(burnin)*length(theta1)*length(theta2)*length(chrom_num)
  
## colnames vector
  columns <- c("seed", "mu_base", "mu_inv", "sigmaK", "alpha", "rep", "N", "en_var", "mig1", "mig2", "theta1", "theta2", "burnin", "chrom_num", "r")
            ...
            for(n in 1:length(N)){
              for(e in 1:length(envar)){
                for(m in 1:length(mig1)){
                  new.row <- c(mu_base[b], mu_inv[i], sigma_k[k], alpha_sd[a],
                              reps[r], N[n], en_var[e], mig1[m], mig2[m], theta1, 
                              theta2, burnin, chrom_num, recom)
...
head(df.params)
write.csv(df.params, "fullINVparamslist.csv", row.names = FALSE)
test.data <- head(df.params)
write.csv(test.data, "testINVparamslist.csv", row.names = FALSE)
