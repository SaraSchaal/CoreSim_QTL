...
if(df.params$muInv == 0){
  ...
  no.inv.manh.plot <- ggplot(df.no.inv.muts.MAF, aes(x = position, y = FST, 
                                       group =  chrom)) +
...
### Critical Migration Plots
  ...
  if(length(adapt.inv) != 0){
    ...
    print(ggarrange(alpha_FST_adapt,
                    alpha_FST_nonadapt, 
                    alpha_FST_outside, 
                    alpha_FST_NS,ncol = 4))
    dev.off()
    ...
    print(ggarrange(alpha_FST_nonadapt,
                    alpha_FST_outside, 
                    alpha_FST_NS, ncol = 3))
    dev.off()
  }
...
## Local adaptation
  LA.title <- bquote(atop(paste(bold("Local Adaptation: "), italic(m), " = ", .(df.params$mig1), 
                          ...
                          atop(textstyle(paste(" ", mu, " = ", .(format(df.params$muProp, scientific = T, digits = 2)), 
...
#   dim(G1_alpha) 
#   G1_alphaFST <- G1_alpha[positionSub,]*df.mutsMAFord$FST[positionSub]
#   G2_alphaFST <- G2_alpha[positionSub,]*df.mutsMAFord$FST[positionSub]
#   
#   png(paste0(folderOut, seed, "_heatmapPop1alphaFST.png"), type = "cairo", width = 7, height = 7, units = 'in', res = 300)
#      heatmap(t(G1_alphaFST[, pop1_order]),   
...
#   dev.off()
#   
#   png(paste0(folderOut, seed, "_heatmapPop1alpha.png"), type = "cairo", width = 7, height = 7, units = 'in', res = 300)
#     heatmap(t(G1_alpha[positionSub, pop1_order]),   
...
#     heatmap(t(G2_alphaFST[, pop2_order]),   
...
#   dev.off()
#   
#   png(paste0(folderOut, seed, "_heatmapPop2alpha.png"), type = "cairo", width = 7, height = 7, units = 'in', res = 300)
#     heatmap(t(G2_alpha[positionSub, pop2_order]),   
...
#### Manhattan plots ####
  ...
  png(paste0(folderOut, seed, "_manhFSTzoom.png"), type = "cairo", width = 25, height = 4, units = 'in', res = 300)
     print(manh.plot + scale_y_continuous(expand = c(0, 0), limits = c(0, min(c(1,max(df.neutQTNmuts$FST)*1.1)))))
  dev.off()
