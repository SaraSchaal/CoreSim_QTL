...
df.SeliWQTNsM <- rbind(df.SeliWMutsQTNs, df.SelMutsNeut)
df.noSeliWQTNsM <- rbind(df.noSeliWMutsQTNs, df.noSelMutsNeut)
...
df.SelQTNsM$FST <- as.numeric(as.character(df.SelQTNsM$FST))
df.noSelQTNsM$FST <- as.numeric(as.character(df.noSelQTNsM$FST))
