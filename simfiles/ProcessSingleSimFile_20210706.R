...
# this is doing SNP pruning - removing correlated SNPs
  newpc <- try(snp_autoSVD(G=G_coded, infos.chr = as.integer(training$chromosome),
  ...
  if("try-error" %in% class(newpc.NS) | "try-error" %in% class(newpc)){
    pcadapt.fail <- TRUE
  } else {
...
### PCA loadings for pruned data ####
  ...
  test.NS <- try(snp_gc(snp_pcadapt(training.NS$G_coded, U.row = newpc.NS$u[,1])))
...
if(length(adapt.inv) != 0){
  ...
  if("try-error" %in% class(out_pruned)){
    ...
    if( outflank.test == TRUE | "try-error" %in% class(outflank.test)){
    ...
    } # if statement for error in outflank
...
if(length(adapt.inv) != 0 ){
  if(pcadapt.fail == TRUE | outflank.fail == TRUE){
    false_neg_pcadapt <- NA
    true_pos_pcadapt <- NA
    true_neg_pcadapt <- NA
    false_pos_pcadapt <- NA
...
if(length(adapt.inv) != 0){  
  if(pcadapt.fail == TRUE | outflank.fail == TRUE){
    true_neg_pcadapt_NS <- NA
    false_pos_pcadapt_NS <- NA
  } else if("try-error" %in% class(test.NS) | "try-error" %in% class(test.NS)){
    true_neg_pcadapt_NS <- NA
    false_pos_pcadapt_NS <- NA
  }
} else {
