...
### Load Packages and Download Data Files ###
  ...
  folderOut <- "figures/20220302_reviews/10gen/" 
  seed <-  "3384725"
...
#### Calculate Inversion Windows ####
  ...
  df.invDataFinalGen <- filter(df.invAllData, sim_gen == 60000)
  df.invDataFinalGen.NS <- filter(df.invAllData.NS, sim_gen == 60000)
...
### Identify adaptive inversions ####
 ...
 # mtext(expression(bold("Frequency")), side = 2, outer = TRUE)
  ...
  total.perc 
  #sanity check
  hist(inQTNs$selCoef, breaks = 50, main = "QTNs inside inversions")
  abline(v= mean(inQTNs$selCoef), col = "red", lty = "dashed", lwd = 3)
  hist(outQTNs$selCoef, breaks = 50, main = paste0("QTNs outside inversions"))
  abline(v = mean(outQTNs$selCoef), col = "red", lty = "dashed", lwd = 3)
...
#### plot inversion origin dynamics ####
  ...
  plot.inv.orig.col <- ggplot(df.pop1[df.pop1$adaptInv == "Adaptive",], aes(x = sim_gen, y = sum_qtnSelCoef)) +
    ...
    geom_line(aes(color = "black", group =  inv_id), alpha = 0.8) + 
    ...
    geom_line(data = df.pop2[df.pop2$adaptInv == "Adaptive",], aes(x = sim_gen, y = sum_qtnSelCoef, color = "black", 
  ...
  plot.inv.orig.col.NA <- ggplot(df.pop1[df.pop1$adaptInv == "Nonadaptive",], aes(x = sim_gen, y = sum_qtnSelCoef)) +
    ...
    geom_point(data = df.pop2[df.pop2$adaptInv == "Nonadaptive",], aes(x = sim_gen, y = sum_qtnSelCoef, fill = inv_id, 
                                   size = inv_FST), shape = 21, alpha = 0.8, inherit.aes = FALSE) + 
    geom_line(data = df.pop2[df.pop2$adaptInv == "Nonadaptive",], aes(x = sim_gen, y = sum_qtnSelCoef, color = inv_id, 
                                  group = inv_id), alpha = 0.8, inherit.aes = FALSE) + 
  ...
  plot.inv.orig.col.NS <- ggplot(df.pop1.NS, aes(x = sim_gen, y = sum_qtnSelCoef)) +
    ...
    geom_line(data = df.pop2.NS, aes(x = sim_gen, y = sum_qtnSelCoef, color = inv_id, group = inv_id), 
...
#### plot heatmaps
  ...
  #hist(G_pop1*a2)
#type = "cairo", 
  # png(paste0(folderOut, seed, "_heatmapPop1alphaFST.png"), width = 7, height = 7, units = 'in', res = 300)
  ...
  #  dev.off()
  # type = "cairo"
  # png(paste0(folderOut, seed, "_heatmapPop2alphaFST.png"), width = 7, height = 7, units = 'in', res = 300)
