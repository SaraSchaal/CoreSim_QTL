...
# download data
 folder <- "results/Inversion/20210321_runLowMig/"
...
#############################################################################################################
  ...
  ## plot inversion age --  split by inversiion FST with reps
    ...
    ### SELECTION ###
    df.invage <- pivot_longer(df.FSTsplit[, c(1,2,7)], cols = c(inv_ageT10, inv_ageB90), #, sd_inv_ageT10, sd_inv_ageB90),
...
#######################################   
  # Selection
    ...
    inv.qtns.Lscaled.plot <- ggplot(data = df.invQTNsLscaled, 
                               ...
                               geom_vline(xintercept = 10000, linetype = "dashed", color = "black") +
                               labs(title = "",
                                     y = "Average Number of inversion QTNs scaled by Inversion Length",
                                     x = "Generation") +
                               theme_classic() +
                               theme(panel.background = element_blank(), 
                                     strip.background = element_rect(colour = "white", fill = "grey92")) +
...
## Plotting
par(mfrow = c(1,3))
plot(invOrigNumQTNs~inv_originGen, data = df.InvDataOriginMAF, xlab = "Inversion Origin Generation",
     ylab = "Origin number of QTNs in Inversion", col = "cornflowerblue", pch = 19, ylim = c(0, 400))
plot(num_qtns~inv_originGen, data = df.InvDataOriginMAF, xlab = "Inversion Origin Generation",
     ylab = "Final number of QTNs in Inversion", col = "dodgerblue4", pch = 19, ylim = c(0, 400))
plot(invOrigQTNSelCoef~inv_originGen, data = df.InvDataOriginMAF, xlab = "Inversion Origin Generation",
...
## this clusters individuals by genotype using ward.D clustering
```{r}
