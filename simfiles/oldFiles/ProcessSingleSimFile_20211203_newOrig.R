...
### Load Packages and Download Data Files ###
  ...
  folderIn <- "results/Inversion/20211203_finalRerun/"
  folderOut <- "figures/20211203_finalRerun/" 
  seed <-  "3384725"
...
#### plot inversion origin dynamics ####
  # Subset for final generation
  df.invFinalGen <- subset(df.invHapTime, subset = sim_gen == 50000)
  df.InvDataOrigin <- left_join(df.invFinalGen, df.invData, by = "inv_id")
  df.invFinalGen.NS <- subset(df.invHapTime.NS, subset = sim_gen == 50000)
  ...
  max_value_origin <- max(c(df.invFinalsubset$sum_qtnSelCoef, df.invFinalsubset.NS$sum_qtnSelCoef))
